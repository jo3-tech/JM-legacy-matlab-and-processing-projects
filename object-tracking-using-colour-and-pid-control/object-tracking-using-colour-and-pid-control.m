...
NXT_PlayTone(440, 500);                 %play a tone on joe-nxt to confirm a connection
...
Amin=1500;       %minimum area the object must be before the robot recognises it as 'properly in view'
xMAX=240;
xMIN=0;
...
offset=(xMIN+xMAX)/2;   %average of min and max x-values =120; hence errorMIN = 0-120=-120 & errorMAX=240-120=120
Tp=30;              %target power; when error=0(target directly in front); i want the robot to move forward with targetpower=30
Kp=1/6;             %slope; Kp=turn/error; power=Tp+turn; when abs error is max i.e. 120 or-120 i want robot to turn at power of 50,
                    %so turn=50-30=20 & Kp=20/120=1/6
...
%%
    ...
    %check if an object is in the scene & follow it; start
    if numObjects==0
    ...
    %check if an object is in the scene & follow it; end
