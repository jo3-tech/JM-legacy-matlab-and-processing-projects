//==LIBRARIES============================================================================================================================
//---------------------------------------------------------------------------------------------------------------------------------------
import apwidgets.*;  // to playback audio 
import android.view.WindowManager;  // to access commands that prevent screen from sleeping/backlight going off
...
//---------------------------------------------------------------------------------------------------------------------------------------
APMediaPlayer growl; // declare variable to store 'growl' audio
APMediaPlayer scream;// declare variable to store 'scream' audio
//---------------------------------------------------------------------------------------------------------------------------------------
...
void setup()
{
  ...
  //size(displayWidth,displayHeight);         // set screen size                     NOT REALLY NEEDED BECAUSE ANDROID DOES THIS AUTOMATICALLY
  ...
  growl=new APMediaPlayer(this);            // initialise APMediaPlayer variables
  scream=new APMediaPlayer(this);           // initialise APMediaPlayer variable
  
  growl.setMediaFile("growl.mp3");          // load media files 
  scream.setMediaFile("scream.mp3");        // load media files
...
void draw()
{
  ...
  switch(control)
  {
    ...
    if((millis()-iT)>=sDuration)            // display 'scary' image after specified duration
    {
      ...
      if(playing==false)
      {
        growl.start();                      // start media play back
        growl.setLooping(false);            // do not restart playback when end reached
        growl.setVolume(lvol, rvol);        // set left and right volumes. range is from 0.0 to 1.0
        
        scream.start();                     
        scream.setLooping(false);           
        scream.setVolume(lvol, rvol);
...
public void onDestroy() 
{
  ...
  //release the players
  if(growl!=null)   // must be checked because or else crash when return from landscape mode 
  { 
    growl.release();
  }
  
  if(scream!=null)   // must be checked because or else crash when return from landscape mode 
  { 
    scream.release();
  }
}
