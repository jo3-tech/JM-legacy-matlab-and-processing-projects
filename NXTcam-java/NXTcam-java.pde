//==SKETCH PERMISSIONS===================================================================================================================
//you must tick bluetooth, bluetooth_admin, camera & write_external_storage for android mode!
...
//---------------------------------------------------------------------------------------------------------------------------------------
String service_name;   // Service name="Dev A" (incoming from NXT), Service name="Dev B" (outgoing to NXT), server name="Joe-NXT";
...
//  jm_GWsize("galaxys3","landscape","display");    //*****************testing
  ...
  File fC=new File(dataPath("colour.txt"));
...
//    aveRGB[0]=132.37625; aveRGB[1]=129.96645; aveRGB[2]=66.46748; T=99;       // yellow using the pc philips spc230nc webcam image
  ...
  File fS=new File(dataPath("service.txt"));
  
  if(fS.exists()){ String service[]=loadStrings(dataPath("service.txt")); service_name=service[0]; }
  else { service_name = "Dev B"; saveStrings(dataPath("service.txt"),new String[]{service_name}); }
  
  btMSG.jm_TBsetName(jm_BTmessage()+": "+service_name);
...
if((CTRL!=start_tracking)&&(CTRL!=control_end)&&(CTRL!=bt_messaging)&&(CTRL!=anim_intro)&&(CTRL!=test_screen))
{
...
case anim_intro:
  ...
  if(currentT>=introT/2) { /*do nothing*/ }    // in android mode blurring seems to take up a lot of processing power & hence takes longer than what I specified :-S
  else
  {
...
case home:
  ...
  { 
    btMSG.jm_TBsetName("nxt service name: "+service_name);                // reset bluetooth & blob analysis message
  ...
  jm_BTstop(); //you also need to stop bluetooth here in android mode, don't think this matters for java mode... but we just use a variable to keep track of the connection
  ...
  for (int i=0; i<home_buttons.length; i++)
  {
    ...
    if(mouse_clicked)
    {
      if(home_buttons[i].jm_TBmouseOver(wSF,hSF))
      {
        ...
        if(CTRL==define_colour){ input_finished=false; keyboard_input=int(aveRGB[0])+","+int(aveRGB[1])+","+int(aveRGB[2])+","+int(T); } // hide the decimals, dont need to use str(..), seems it converts to string automatically
        else if(CTRL==define_device){ input_finished=false; keyboard_input=service_name; } //NOTE:you need to turn on keyboard here for android if CTRL==define_device OR CTRL==define_colour; keyboard_active=true;
...
case define_device:
  ...
  if(input_finished){ saveStrings(dataPath("service.txt"),new String[]{keyboard_input}); service_name=keyboard_input; CTRL=home; } 
  ...
  //NOTE:
  //here you need to check if the text field was touched 
  //and show the keyboard if so
  //this is in case the user presses the back key 
...
case bt_finished:
  ...
  boolean rtn=jm_BTconnect(service_name); btMSG.jm_TBsetName(jm_BTmessage());  
...
//NOTE: 
//we don't really need to wait for a proper connection in
//java mode either for the cam or bluetooth, but i'll just   
//leave this here anyways.. the loading screen is cool :-P

//NOTE: cannot start the cam here in java mode, it must be done in setup      
...
//  image(oSEGI,0,0);                     // causes OutOfMemoryError error after a while although i think this is now fixed using statement at end of tracking; avoid using except when testing*****FOR TESTING*****  
...
//    cmd[6]=byte(126); println(cmd[6]); jm_BTsend(cmd);                        // ********TESTING*****
  }
  
  else
  {
//    cmd[6]=byte(233); println(cmd[6]);           //FOR TESTING**** note; it may print a negative number (not sure why!!!) but it sends the correct number to nxt    
...
//    println("orio x= "+ jm_BAblobx());
    ...
    if(jm_BTconnected()){ jm_BTsend(cmd); }  // send bt message to nxt indicating location (x-coordinate), nxt should turn left or right &/or move forward towards this location
  }
...
/*
NOTE:
in android there is an extra method here for preventing the program from quitting automatically
*/
//---------------------------------------------------------------------------------------------------------------------------------------
/*
NOTE:
in android; 
function for showing soft/virtual keyboard 
*/
//---------------------------------------------------------------------------------------------------------------------------------------
/* 
NOTE:
...
in android;
onCreate() function
where bluetooth object is created & a method for keeping the screen from sleeping is activated 
*/
//---------------------------------------------------------------------------------------------------------------------------------------
/* 
NOTE:
in android; onActivityResult() function where some bluetooth stuff is done
*/
//---------------------------------------------------------------------------------------------------------------------------------------
/*
NOTE:
